...
namespace Stockfish::Learner
{
        ...
        int& random_move_c)
    {
            ...
            (params.random_move_minply == -1 && random_move_c < params.random_move_count))
        {
			...
			if (node!=nullptr)
			{
				expTTMove = node->siblingMoveInfo[rand()%node->siblingMoveInfo.size()].move;
			}
			if(expTTMove && rand() % 100 > 10)
			{
				for(size_t i= 0; i < list.size(); i++)
				  if(list.at(i) == expTTMove)
				  {
					  expHit = true;
					  random_move = list.at(i);
					  //cout << "Experience! : " << pos.fen() << i << endl;
					  break;
				  }
			}

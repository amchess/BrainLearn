...
BrainLearnDGV is a special version, derived from nodchip repository ([https://github.com/nodchip/Stockfish]), 
used to generate and validate data also based on an experience file. See the word documents for more information ([https://gitlab.com/amchess/brainlearn/-/tree/DGV/docs]).
...
This is a particular case of the general reinforcement learning. The procedure is the following:

a. Generation
   
   The following uci commands must be run using an interactive prompt after the engine has been started or by the [nnue-gui](https://github.com/FireFather/nnue-gui/tags). 

Create a folder with the engine executable + the original net you wants to improve + the experience.bin file and execute the following commands in the specified order.
...
setoption name Threads value <usual max available threads number>
setoption name Hash value <usual max available hash size in MB>
setoption name SyzygyPath value <sygyzy path>
setoption name  SyzygyProbeDepth value <best sygyzy probe path
gensfen depth 13 loop 100000000 save_every 10000000 
```

At the end, it will be generated a file named *generated_kifu.binpack*. 

Rename this file in *100millionsdepth13.binpack* and move it to a new subfolder of the engine folder named *trainingdata*. 
 
   2. Validation data (not mandatory)

       Repeat the same steps as 1 (generation data), but with the following *gensfen* command:
```sh
gensfen depth 14 loop 10000
```

At the end, rename the generated file in *val.binpack* and put it in a new subfolder named *validationdata*.

b. Network training by reinforcement learning from experience.bin file
...
setoption name SyzygyPath value <sygyzy path>
setoption name  SyzygyProbeDepth value <best sygyzy probe path
learn targetdir trainingdata epochs 10000 eval_save_interval 250000000  newbob_num_trials 10 max_grad 0.5 validation_set_file_name validationdata\val.binpack
...
The idea is to saturate the original network by the experience.bin file.
